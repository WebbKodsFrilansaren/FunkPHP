...
{
    // Remove any invalid UTF-8 characters from possibly parsed text strings
    $textStringSanitizerHandler = function ($str) {
        if (!is_string($str)) {
            throw new Exception("CBOR text value must be string");
        }
        // Validate UTF-8 without altering data
    ...
    if ($addInfo < 24) {
        ...
        throw new Exception("Unsupported CBOR length encoding. Currently only supports lengths up to 4 bytes.");
    }
    ...
    switch ($majorType) {
        ...
        case 3: // text string
            ...
            $offset += $value;
            return $textStringSanitizerHandler($text);

        case 4: // array
            $arr = [];
            for ($i = 0; $i < $value; $i++) {
                $arr[] = funk_cbor_decode_simplified_for_passkeys($data, $offset);
            }
            return $arr;
        ...
        case 5: // map (key-value pairs)
            $map = [];
            for ($i = 0; $i < $value; $i++) {
                $key = funk_cbor_decode_simplified_for_passkeys($data, $offset);
                $val = funk_cbor_decode_simplified_for_passkeys($data, $offset);
                $map[$key] = $val;
            }
            return $map;

        default:
            throw new Exception("Unsupported major type. It should only be between 0-5.");
    }
}
