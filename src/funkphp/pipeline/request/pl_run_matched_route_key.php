...
return function (&$c, $key) {
    ...
    if (!isset($c['req']['route_keys'][$key])) {
        $c['err']['PIPELINE']['REQUEST']['funk_run_matched_route_key'][] = 'Route Key `' . $key . '` NOT found for the Route `' . ($c['req']['route'] ?? '<No Route Matched>') . '`. Please check your Route Keys in `funkphp/routes/routes.php` for the Route `' . ($c['req']['method'] ?? '<No HTTP(S) Method Matched>') . ($c['req']['route'] ?? '<No Route Matched>') . '`!';
        return;
    }
    ...
    if (isset($c['dispatchers'][$key][$keyFile])) {
        ...
        if (is_callable($c['dispatchers'][$key][$keyFile])) {
            ...
            $c['err']['PIPELINE']['REQUEST']['funk_run_matched_route_key'][] = 'Route Key `' . $key . '` File `' . $keyFile . '` is NOT a Callable Function. Please check your Route Key File in `funkphp/routes/routes.php` for the Route `' . ($c['req']['method'] ?? '<No HTTP(S) Method Matched>') . ($c['req']['route'] ?? '<No Route Matched>') . '`!';
            return;
        }
    }
    // Not added yet so add if it exists and call it with the $keyFn!
        ...
        if (!is_readable($pathToInclude)) {
            $c['err']['PIPELINE']['REQUEST']['funk_run_matched_route_key'][] = 'Route Key `' . $key . '` File `' . $keyFile . '` does NOT EXIST in `funkphp/routes/' . $keyFolder . '/` Directory! Please check your Route Key File in `funkphp/routes/routes.php` for the Route `' . ($c['req']['method'] ?? '<No HTTP(S) Method Matched>') . ($c['req']['route'] ?? '<No Route Matched>') . '`!';
            return;
        }
