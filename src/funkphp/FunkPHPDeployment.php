...
namespace {
    ...
    function funk_run_pipeline_request(&$c)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $c['req']['next_pipeline'] = isset($c['<ENTRY>']['pipeline']['request'][$i + 1]) && is_array($c['<ENTRY>']['pipeline']['request'][$i + 1]) ? array_key_first($c['<ENTRY>']['pipeline']['request'][$i + 1]) : null;
        }
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if (isset($c['dispatchers']['pipeline']['post_response'][$fnToRun])) {
                    if (is_callable($c['dispatchers']['pipeline']['post_response'][$fnToRun])) {
                        ...
                        $c['req']['next_pipeline'] = isset($c['<ENTRY>']['pipeline']['post_response'][$i + 1]) && is_array($c['<ENTRY>']['pipeline']['post_response'][$i + 1]) ? array_key_first($c['<ENTRY>']['pipeline']['post_response'][$i + 1]) : null;
                        $rawRun = $runPipeKey($c);
                    } else {
    ...
    function funk_match_developer_route(&$c, string $method, string $uri, array $compiledRouteTrie, array $developerSingleRoutes)
    {
        ...
        if ($routeDefinition !== null) {
            ...
            if (isset($developerSingleRoutes[$method][$routeDefinition["route"]])) {
                ...
                if (isset($routeDefinition["middlewares"]) && is_array($routeDefinition["middlewares"]) && !empty($routeDefinition["middlewares"])) {
                    foreach ($routeDefinition["middlewares"] as $middleware) {
                        if (isset($developerSingleRoutes[$method][$middleware]) && isset($developerSingleRoutes[$method][$middleware]['middlewares'])) {
