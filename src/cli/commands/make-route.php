...
if (!isset($ROUTES[$method][$route])) {
    cli_err("`$method$route` does suddenly NOT EXIST in `funkphp/core/pipeline_routes.php` even though it should since we just created it earlier if it did not exist before. Command stopped without creating/adding the Route Key `$singleFolder=>$file=>$fn`!");
}
if (!is_array($ROUTES[$method][$route]) || !array_is_list($ROUTES[$method][$route])) {
    cli_err("`$method$route` in `funkphp/core/pipeline_routes.php` is NOT a Numbered Array even though it should be. Command stopped without creating/adding the Route Key `$singleFolder=>$file=>$fn`!");
}
