...
if (!isset($ROUTES[$method][$route])) {
    cli_err("Suddenly, the Method/Route `$method$route` does not exist in `funkphp/routes/routes.php` even though it should since we just created it earlier if it did not exist before. Please report this bug to `https://www.GitHub/WebbKodsFrilansaren/FunkPHP`!");
}
if (!is_array($ROUTES[$method][$route]) || !array_is_list($ROUTES[$method][$route])) {
    cli_err("The Method/Route `$method$route` in `funkphp/routes/routes.php` is NOT a Numbered Array even though it should be. Please report this bug to `https://www.GitHub/WebbKodsFrilansaren/FunkPHP`!");
}
...
if (count($ROUTES[$method][$route]) > 0) {
    if (!cli_duplicate_folder_file_fn_route_key($ROUTES[$method][$route], $folder, $file, $fn)) {
