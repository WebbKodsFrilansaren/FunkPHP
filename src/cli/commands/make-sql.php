...
if (!$statusArray['file_exists']) {
    cli_info_without_exit("SQL File `$file.php` does not exist in `funkphp/data/sql`, it will be created along with SQL Function `$fn` inside of it...");
    $createStatus = cli_crud_folder_and_php_file($statusArray, "create_new_file_and_fn", $file, $fn, "sql", null, $tablesProvided);
    if ($createStatus) {
        cli_success_without_exit("SUCCESSFULLY Created SQL File `$file.php` with SQL Function `$fn` in `funkphp/data/sql`!");
        cli_info_without_exit("The SQL File `$file.php` is now ready to be used in `funkphp/data/sql`.");
        cli_info("Use it in your Route Function Files in `funkphp/pipeline/routes/{files}` by calling `funk_load_sql(&\$c, '$file', '$fn')` and then `funk_use_sql(&\$c, \$loadedSQLArray, \$optionalInputData, \$hydrateAfterQuery)`!");
    } else {
        cli_err("FAILED to Create SQL File `$file.php` with SQL Function `$fn` in `funkphp/data/sql`!");
    }
}
// Or try to create a new function in existing file
else {
    // Function already exists in the file
    cli_info_without_exit("SQL File `$file.php` already exists in `funkphp/data/sql`, SQL Function `$fn` will be created inside of it unless it already exists...");
    if (isset($statusArray['functions'][$fn])) {
        cli_err_without_exit("SQL Function `$fn` already exists in SQL File `$file.php` in `funkphp/data/sql`!");
        cli_info("Change SQL File and/or SQL Function Name and try again for `funkphp/data/sql`!");
    }
    ...
    // crudType "create_only_new_fn_in_file"
        ...
        if ($createStatus) {
            ...
            cli_err("FAILED to Create SQL Function `$fn` in SQL File `$file.php` in `funkphp/data/sql`!");
        }
    }
}
