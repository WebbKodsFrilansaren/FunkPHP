...
function cli_dump_with_ignore_depths($var, $exit = true, $horisontal = false, array $ignoreSpecificKeyDepths = [])
{
    ...
    $cStatLbl = "\e[0;37m"; // White for Stat labels
    $cStatVal = "\e[1;32m"; // Bold Green for counts
    // Transform ignored paths array into O(1) hash lookup table
    $ignoreMap = !empty($ignoreSpecificKeyDepths) ? array_fill_keys($ignoreSpecificKeyDepths, true) : [];
    // Global Value Type count metrics
    $globalTypeCount = [
        'nulls'          => 0,
        'strings'        => 0,
        'strings-empty'  => 0,
    ...
    echo "{$cLine}" . str_repeat('=', 150) . "{$cReset}\n";
    // Isolated recursive formatting closure engine
    $render = function ($data, $depth = 0, $keyName = null, $inList = false, $currentPath = '') use (
        &$render,
        $cReset,
        $cKey,
        $cIndex,
        ...
        $indent = $horisontal ? str_repeat(' ', $depth) : str_repeat('   ', $depth);
        $prefix = '';
        // 1. Render the incoming accessor prefix (Key name vs list index position)
        if ($keyName !== null) {
            $prefix = $inList
        ...
        if (is_array($data)) {
            ...
            if ($count === 0) {
                ...
                if (!$horisontal) echo "\n";
                foreach ($data as $k => $v) {
                    // Compute current dot-notation path
                    $childPath = ($currentPath === '') ? (string)$k : "{$currentPath}.{$k}";
                    // Skip ignored paths dynamically
                    if (isset($ignoreMap[$childPath])) {
                        continue;
                    }
                    $render($v, $depth + 1, $k, $isList, $childPath);
                }
                echo "{$indent}{$cType}] // {$keyName} ({$arrType}, {$count}) {$cReset}\n";
            }
        } elseif (is_object($data)) {
            ...
            echo "{$prefix}{$cType}Object('{$className}') ({$count}) {{$cReset}\n";
            foreach ($properties as $k => $v) {
                // Strip null-byte markers to get clean property key for path matching
                $cleanKey = str_replace("\0*\0", '', $k);
                $cleanKey = preg_replace('/^\0[^\0]+\0/', '', $cleanKey);
                $childPath = ($currentPath === '') ? $cleanKey : "{$currentPath}.{$cleanKey}";
                // Skip ignored paths dynamically
                if (isset($ignoreMap[$childPath])) {
                    continue;
                }
                // Format key display label for output
                $displayKey = str_replace("\0*\0", '(protected) ', $k);
                $displayKey = preg_replace('/^\0[^\0]+\0/', '(private) ', $displayKey);
                $render($v, $depth + 1, $displayKey, false, $childPath);
            }
            echo "{$indent}{$cType}}{$cReset}\n";
        } elseif (is_string($data)) {
            $globalTypeCount['strings']++;
            ...
            // Safe fallback for unprintable CLI primitives (like resource descriptors)
            $type = gettype($data);
            if (!isset($globalTypeCount['others'][$type])) {
                $globalTypeCount['others'][$type] = 0;
            }
            $globalTypeCount['others'][$type]++;
            echo "{$prefix}{$cNull}[Type: {$type}]{$cReset}";
            if (!$horisontal) echo "\n";
    ...
    $render($var);
    // Format and output the telemetry metric reporting engine blocks
    $totalOthers = array_sum($globalTypeCount['others']);
    echo "{$cLine}" . str_repeat('=', 150) . "{$cReset}\n";
    echo "{$cHeader}[ DUMP TELEMETRY METRICS SUMMARY ]{$cReset}\n";
    echo "  ├── {$cStatLbl}Structural Containers{$cReset}\n"
        . "  │   ├── Objects:         " . sprintf("{$cStatVal}%-4d{$cReset}", $globalTypeCount['objects']) . "\n"
        ...
        . " {$cType}(True: {$globalTypeCount['booleans-true']} | False: {$globalTypeCount['booleans-false']}){$cReset}\n"
        . "      ├── Null Values:     " . sprintf("{$cStatVal}%-4d{$cReset}", $globalTypeCount['nulls'])       . "\n"
        . "      └── Unmapped Others: " . sprintf("{$cStatVal}%-4d{$cReset}", $totalOthers)                   . "\n";
    if ($exit) {
        exit(1);
    }
}
...
function cli_file_status(string $folder, string $file, bool $useExactFilePathInstead = false, bool $deeperAnalysis = false)
{
...
function cli_file_harvest_all_functions_from_code2(string $code): array
{
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        // 1. Look ahead to find function name (handle return-by-reference like `function &myFunc()`)
        $curr = $i + 1;
        $isByRef = false;
        ...
        // Skip anonymous closures function() {} or invalid definitions
        if ($curr >= $count || $tokens[$curr]->id !== T_STRING) {
            continue;
        }
        $fnName = $tokens[$curr]->text;
        $fnTokenPos = $tokens[$i]->pos;
        // 2. Walk BACKWARD to collect DocBlocks / Comments
        ...
        while ($back >= 0) {
            ...
            break; // Stopped at non-comment code/statement
        }
        $docComment = !empty($collectedComments) ? implode("\n", $collectedComments) : null;
        // 3. Extract Function Arguments: find '(' and matching ')'
        $argStartTok = $curr + 1;
        ...
        // Skip abstract/interface declarations ending with ';'
        ...
        $bodyEndPos = -1;
        $lastTokenIdx = $i;
        // 5. Walk FORWARD through body to analyze inner code & find closing '}'
        for ($j = $bodySearchTokIdx; $j < $count; $j++) {
            ...
            // Detect INNER named function declarations (forbidden!)
            if ($hasStartedBody && $braceDepth > 1 && $tok->id === T_FUNCTION) {
                $next = $j + 1;
                while ($next < $count && ($tokens[$next]->id === T_WHITESPACE || $tokens[$next]->text === '&')) {
                    $next++;
                }
            ...
            // Detect dangerous tokens inside body
            if ($hasStartedBody && $braceDepth >= 1) {
                if ($tok->id === T_EVAL) {
                    $hasEval = true;
                }
                if ($tok->id === T_STRING && in_array(strtolower($tok->text), $dangerousFuncs, true)) {
                    $hasDangerousCalls = true;
                }
            }
            // Hit closing brace of outer function body
        ...
        if ($bodyEndPos === -1) {
            continue; // Malformed PHP body
        }
        // 6. Slice exact code strings using byte offsets
        ...
        // Move outer loop index past this function body
        $i = $lastTokenIdx;
    }
    return $harvested;
}
...
// and then the `use <what>,<what2>,<and_so_on>;` from a given file!
// Function is used in tandem with cli_file_status().
function cli_harvest_namespace_and_uses_from_code(string $code): array
{
    $tokens = PhpToken::tokenize($code);
    $count = count($tokens);
    $namespace = null;
...
function cli_harvest_all_functions_from_code(string $code): array
{
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        $analysis = cli_analyze_body_tokens($bodyRaw, $startLine);
...
function cli_harvest_all_classes_from_code(string $code): array
{
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if ($tok->id === T_CLASS) {
            ...
            // Walk BACKWARD to capture DocComments
            ...
            $docComment = !empty($collectedComments) ? implode("\n", $collectedComments) : null;
            // Find class body opening '{' and check for 'extends'
            ...
            $bodyEndPos = -1;
            $lastTokenIdx = $i;
            // Walk body to detect safety metrics and closing brace
            for ($j = $bodySearchIdx; $j < $count; $j++) {
                ...
                if ($hasStartedBody && $classBraceDepth >= 1) {
                    ...
                    // Catch early exits: exit, die, T_EXIT
                    if ($ctok->id === T_EXIT) {
                        $hasExit = true;
                    }
            ...
            $harvested[$className] = [
                ...
                'class_starts_with_funk_validate'  => str_starts_with(strtolower($className), 'funk_validate_'),
            ];
            $i = $lastTokenIdx;
        }
    }
